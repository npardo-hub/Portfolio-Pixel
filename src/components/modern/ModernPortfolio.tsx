    ...
    <div style={customStyle} className="min-h-screen bg-[var(--bg)] text-[var(--text)] font-jakarta font-light selection:bg-[var(--accent)] selection:text-[var(--bg)] overflow-x-hidden transition-colors duration-500 relative">
      ...
      <section id="educacion" className="py-24 px-6 max-w-6xl mx-auto">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {t.about.education.map((edu, idx) => (
              <div key={idx} className="p-10 rounded-[2rem] border transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl" style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)' }}>
      ...
      <section id="proyectos" className="py-24 px-6 max-w-6xl mx-auto">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {t.projects.map((proj) => (
               <a href={proj.link} target="_blank" rel="noopener noreferrer" key={proj.id} className="group rounded-[2.5rem] border overflow-hidden flex flex-col hover:shadow-2xl hover:-translate-y-2 transition-all duration-500" style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)' }}>
      ...
      <section id="analisis" className="py-24 px-6 max-w-6xl mx-auto">
         ...
         <div className="grid grid-cols-1 md:grid-cols-6 gap-6 md:auto-rows-[250px]">
            {t.infographics.items.map((item, i) => (
               <div key={i} className={`relative border rounded-[2rem] overflow-hidden group cursor-pointer ${i === 0 ? 'md:col-span-4' : 'md:col-span-2'}`} style={{ backgroundColor: 'var(--surface)', borderColor: 'var(--border)' }}>
